...
def test_compose_mounts_runtime_configuration_for_api_and_worker() -> None:
    ...
    for service_name in ("api", "worker"):
        ...
        assert all("taxonomy.yaml" not in volume for volume in volumes)


def test_docker_image_contains_migration_and_default_vertical_assets() -> None:
    """镜像必须包含迁移和默认 vertical，且不能复制已移除的旧 taxonomy。"""

    dockerfile = (PROJECT_ROOT / "Dockerfile").read_text()
    assert "COPY alembic.ini ./" in dockerfile
    assert "COPY migrations ./migrations" in dockerfile
    assert "COPY verticals ./verticals" in dockerfile
    assert "COPY taxonomy.yaml" not in dockerfile
